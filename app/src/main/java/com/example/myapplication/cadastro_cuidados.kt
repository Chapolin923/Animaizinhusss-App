...
import com.example.myapplication.databinding.ActivityCadastroCuidadosBinding
import com.google.android.material.snackbar.Snackbar
import com.google.firebase.auth.FirebaseAuth
import com.google.firebase.firestore.FirebaseFirestore

class cadastro_cuidados : AppCompatActivity() {
    private lateinit var binding: ActivityCadastroCuidadosBinding
    private val db = FirebaseFirestore.getInstance()
    private val auth = FirebaseAuth.getInstance()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityCadastroCuidadosBinding.inflate(layoutInflater)
            ...
            if(id.isEmpty() || especi.isEmpty()){
                ...
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if (documento != null) {
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("cuidado1", id)
                        }
                    }
