...
class alterar_animal : AppCompatActivity() {
    private lateinit var binding: ActivityAlterarAnimalBinding
    private val db = FirebaseFirestore.getInstance()
    private val auth = FirebaseAuth.getInstance()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAlterarAnimalBinding.inflate(layoutInflater)
        ...
        binding.button.setOnClickListener {view ->
            ...
            if(idade.isEmpty() || nome.isEmpty() || raca.isEmpty() || peso.isEmpty()) {
                ...
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("nome",nome)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("idade",idade)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("raca",raca)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("peso",peso)
        ...
        binding.button3.setOnClickListener {
            db.collection("Usuarios").document(email).update("pet1","999")
            val intent = Intent(this, despertador::class.java)
            startActivity(intent)
        }
    }
