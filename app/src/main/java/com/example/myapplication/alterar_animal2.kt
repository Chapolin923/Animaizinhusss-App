...
class alterar_animal2 : AppCompatActivity() {
    private lateinit var binding: ActivityAlterarAnimal2Binding
    private val db = FirebaseFirestore.getInstance()
    private val auth = FirebaseAuth.getInstance()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityAlterarAnimal2Binding.inflate(layoutInflater)
        ...
        binding.button.setOnClickListener {view ->
            ...
            if(idade.isEmpty() || nome.isEmpty() || raca.isEmpty() || peso.isEmpty()) {
                ...
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet2").toString()).update("nome",nome)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet2").toString()).update("idade",idade)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet2").toString()).update("raca",raca)
                        }
                    }
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if(documento != null){
                            db.collection("Animais").document(documento.getString("pet2").toString()).update("peso",peso)
        ...
        binding.button3.setOnClickListener {
            db.collection("Usuarios").document(email).update("pet2","999")
            val intent = Intent(this, despertador::class.java)
            startActivity(intent)
        }
    }
