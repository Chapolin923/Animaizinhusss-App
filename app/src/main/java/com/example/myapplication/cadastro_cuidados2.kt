...
class cadastro_cuidados2 : AppCompatActivity() {
    private lateinit var binding: ActivityCadastroCuidados2Binding
    private val db = FirebaseFirestore.getInstance()
    private val auth = FirebaseAuth.getInstance()
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityCadastroCuidados2Binding.inflate(layoutInflater)
            ...
            if(id.isEmpty() || especi.isEmpty()){
                ...
                db.collection("Usuarios").document(email)
                    .addSnapshotListener{documento, error ->
                        if (documento != null) {
                            db.collection("Animais").document(documento.getString("pet1").toString()).update("cuidado2", id)
                        }
                    }
